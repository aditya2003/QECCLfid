...
def ComputeUncorrProbs(probs, qcodes, nlevels, leading_fraction, recompute=None):
	...
	if probs.ndim == 2:
		pauli_probs = np.prod(probs[range(qcodes[0].N), qcodes[0].PauliOperatorsLST[:, range(qcodes[0].N)]], axis=1)
	else:
		pauli_probs = probs
	...
	if ((leading_fraction > 0) and (leading_fraction < 1)):
		...
		complete_pauli_probs = CompleteDecoderKnowledge(leading_fraction, pauli_probs, qcodes[0], option="dpfill")
	...
	# Compute the TVD between the true and the reconstructed distribution.
	tvd = np.sum(np.abs(complete_pauli_probs - pauli_probs))
	# print("alpha: {}, tvd = {}".format(leading_fraction, tvd))
	...
	for l in range(nlevels):
		# print("complete_pauli_probs = {}".format(complete_pauli_probs[complete_pauli_probs <= 0]))
		if l == 0:
			# Get level-0 contribution -- this is just the probability of I.
			correctable_probabilities[l] = complete_pauli_probs[0]
		
		elif l == 1:
			# Get level-1 contribution
			# These are all errors that are corected by the level-1 code.
			correctable_probabilities[l] = np.sum(
				complete_pauli_probs[qcodes[0].PauliCorrectableIndices]
			)
			# correctable_probabilities[1] = 1 - sum(
			#     [complete_pauli_probs[p] for p in qcode.PauliCorrectableIndices]
			# )
			...
			correctable_probabilities[l] = np.power(correctable_probabilities[l - 1], qcodes[l - 1].N)
			...
			for log in range(4):
				coset_probs[log] = ComputeResiduals(log, complete_pauli_probs, qcodes[l - 2])
			# print("Coset probs = {}".format(coset_probs))
			correctable_probabilities[l] += np.sum(np.prod(coset_probs[qcodes[l - 1].Paulis_correctable[1:]], axis=1))
